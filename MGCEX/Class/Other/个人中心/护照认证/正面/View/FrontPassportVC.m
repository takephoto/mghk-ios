...
- (void)viewDidLoad {
    ...
    //固定的头部进度条
    IDCardProgressView * progressView = [[IDCardProgressView alloc]initWithImage:@"hzjd2" step:2 style:2 frame:CGRectMake(0, 0, MAIN_SCREEN_WIDTH, Adapted(80))];
    ...
    _headView = [[IDCardPhotographView alloc]initWithPhotograph:@"sczm-hz" titleLabel:kLocalizedString(@"请上传您的护照正面图片") frame:CGRectMake(0, 0, MAIN_SCREEN_WIDTH, Adapted(300))];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.row == 0){
        ...
        cell.pics = @[@"zq-hzzm",@"qszd-hzzm",@"mh-hzzm",@"sg-hzzm"];
        return cell;
    }else{
...
-(void)nextStepClick{
    ...
    [self.viewModel.upImageSignal subscribeNext:^(UpImageModel * model) {
        ...
        [[NSUserDefaults standardUserDefaults]setObject:model.imageName forKey:PassportPositive];
        [[NSUserDefaults standardUserDefaults] synchronize];
        
        HandPassportVC * vc = [HandPassportVC new];
