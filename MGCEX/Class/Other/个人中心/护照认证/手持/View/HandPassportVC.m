...
- (void)viewDidLoad {
    ...
    //固定的头部进度条
    IDCardProgressView * progressView = [[IDCardProgressView alloc]initWithImage:@"hzjd3" step:3 style:2 frame:CGRectMake(0, 0, MAIN_SCREEN_WIDTH, Adapted(80))];
    ...
    _headView = [[IDCardPhotographView alloc]initWithPhotograph:@"sczj_schz" titleLabel:kLocalizedString(@"请上传您的手持护照和平台认证说明") frame:CGRectMake(0, 0, MAIN_SCREEN_WIDTH, Adapted(300))];
...
-(void)nextStepClick{
    ...
    self.nextViewModel.type = @"2";
    self.nextViewModel.identityNum = [[NSUserDefaults standardUserDefaults] objectForKey:PassportNumber];
    self.nextViewModel.surname = [[NSUserDefaults standardUserDefaults] objectForKey:PassportsUrname];
    self.nextViewModel.name = [[NSUserDefaults standardUserDefaults] objectForKey:PassportName];
    self.nextViewModel.frontCardUrl = [[NSUserDefaults standardUserDefaults] objectForKey:PassportPositive];
    self.nextViewModel.holdCardurl = [[NSUserDefaults standardUserDefaults] objectForKey:PassportHand];
