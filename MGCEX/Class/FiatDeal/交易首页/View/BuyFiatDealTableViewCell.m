...
@interface BuyFiatDealTableViewCell()
...
@end
@implementation BuyFiatDealTableViewCell
...
-(void)setUpViews{
    ...
    vipImageV.image = IMAGE(@"icon_vip");
    ...
    [buyBtn setBackgroundColor:kGreenColor forState:UIControlStateNormal];
    buyBtn.layer.cornerRadius = Adapted(2);
    [buyBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(Adapted(-15));
        make.width.mas_equalTo(Adapted(82));
...
-(void)buyClick:(UIButton *)btn{
    if([self.buyDelegate respondsToSelector:@selector(BuyFiatDealWithBuy:)]){
        ...
        [self.buyDelegate BuyFiatDealWithBuy:model];
    }
}
