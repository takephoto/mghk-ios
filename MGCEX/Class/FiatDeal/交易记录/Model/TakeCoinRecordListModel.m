...
@interface TakeCoinRecordListModel ()
/// 高度
@property (nonatomic, readwrite) CGFloat cellHeight;
@end

@implementation TakeCoinRecordListModel
...
- (CGFloat)cellHeight
{
    if (_cellHeight == 0) {
        ...
        // 日期高度
        CGFloat dateHeight = ceil([self.orderTime boundingRectWithSize:CGSizeMake((MAIN_SCREEN_WIDTH-Adapted(30))/3.0, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:H15} context:nil].size.height);
