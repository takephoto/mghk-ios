...
@implementation FiatMaximumCell
/**
 绑定属性
 */
- (void)bindModel{
...
-(void)setUpViews{
    ...
    titleLabel.font = H15;
    titleLabel.text = kLocalizedString(@"交易最大额度");
    ...
    self.maxNumberTextField = textFiled;
    self.maxNumberTextField.limitDecimalDigitLength = s_Integer(8);
    ...
    self.maxAountField = textFiled2;
    self.maxAountField.limitDecimalDigitLength = s_Integer(2);
    ...
    noteLabel.text = kLocalizedString(@"单笔最大限额请根据您实际绑定的银行卡、微信、支付宝限额进行调整");
    [noteLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(Adapted(15));
        make.right.mas_equalTo(Adapted(-15));
        make.top.mas_equalTo(lineView2.mas_bottom).offset(Adapted(13));
        make.bottom.mas_equalTo(Adapted(-15));
    }];
    @weakify(self)
    self.maxNumberTextField.didChangeBlock = ^(NSString *text) {
        @strongify(self)
        if ([self.price doubleValue] > 0) {
            if (self.maxNumberTextField.text.length > 0) {
    ...
    self.maxAountField.didChangeBlock = ^(NSString *text) {
        @strongify(self)
        if ([self.price doubleValue] > 0) {
