...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _mediaArray.count && _mediaArray.count >= _maxImageSelected) {
        NSString* str1 = kLocalizedString(@"最多只能选择");
        NSString * str2 = kLocalizedString(@"张");
        [UIAlertController showAlertWithTitle:[NSString stringWithFormat:@"%@%ld%@",str1,(long)_maxImageSelected,str2] message:nil actionTitles:@[kLocalizedString(@"确定")] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
        return;
    }
    ...
    if (indexPath.row == _mediaArray.count) {
        switch (_type) {
            ...
            {
                ACAlertController *alert = [[ACAlertController alloc] initWithActionSheetTitles:@[kLocalizedString(@"相册"), kLocalizedString(@"相机")] cancelTitle:kLocalizedString(@"取消")];
            ...
            default:
            {
                ACAlertController *alert = [[ACAlertController alloc] initWithActionSheetTitles:@[kLocalizedString(@"相册"), kLocalizedString(@"相机"),kLocalizedString(@"录像"),kLocalizedString(@"视频")] cancelTitle:kLocalizedString(@"取消")];
                [alert clickActionButton:^(NSInteger index) {
                    if (index == 0) {
                        [weakSelf openAlbum];
                    }else if (index == 1) {
...
- (void)openCamera {
    ...
    if ([UIImagePickerController isSourceTypeAvailable: sourceType]){
        ...
        [UIAlertController showAlertWithTitle:kLocalizedString(@"该设备不支持拍照") message:nil actionTitles:@[kLocalizedString(@"确定")] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
    }
}
...
- (void)openVideotape {
    ...
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        [UIAlertController showAlertWithTitle:kLocalizedString(@"当前设备不支持录像") message:nil actionTitles:@[kLocalizedString(@"确定")] cancelTitle:nil style:UIAlertControllerStyleAlert completion:nil];
    }
    [[self currentViewController] presentViewController:picker animated:YES completion:nil];
